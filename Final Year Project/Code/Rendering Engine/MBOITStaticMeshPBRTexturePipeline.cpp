...
void MBOITStaticMeshPBRTexturePipeline::Create(const GraphicsAPI& graphicsAPI, const SwapChain& swapChain, VkRenderPass absorbanceRenderPass, VkRenderPass transmittanceRenderPass, VkDescriptorSetLayout perTransmittancePassDescriptorSetLayout)
{
...
void MBOITStaticMeshPBRTexturePipeline::RenderAbsorbance(VkCommandBuffer commandBuffer, const MeshComponent* meshComponent, const MeshMaterial& meshMaterial, MBOITPushConstants mboitPushConstants, VkSampler sampler)
{
    ...
    if (descriptorSetMaterial == VK_NULL_HANDLE)
    {
        ...
        PBRTextureMaterial* pbrMaterial = static_cast<PBRTextureMaterial*>(meshMaterial.material.get());

        auto albedoImageInfo = pbrMaterial->albedo.GetImageInfo(sampler);
        DescriptorSetBuilder()
            .BindImage(1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &albedoImageInfo)
...
void MBOITStaticMeshPBRTexturePipeline::RenderTransmittanceClustered(VkCommandBuffer commandBuffer, const MeshComponent* meshComponent, const MeshMaterial& meshMaterial, MBOITTransmittanceClusteredConstants mboitTransmittanceConstants, VkDescriptorSet perTransmittancePassDescriptor, VkSampler sampler)
{
    const Mesh& mesh = meshComponent->GetMesh();

    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, transmittanceClusteredPipeline);

    vkCmdPushConstants(commandBuffer, transmittanceClusteredPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(MBOITTransmittanceClusteredConstants), &mboitTransmittanceConstants);
    ...
    if (perMaterialDescriptorSet == VK_NULL_HANDLE)
    {
        ...
        VkDescriptorImageInfo albedo = pbrMaterial->albedo.GetImageInfo(sampler);
        VkDescriptorImageInfo normals = pbrMaterial->normals.GetImageInfo(sampler);
        VkDescriptorImageInfo metalicRougness = pbrMaterial->metalicRougness.GetImageInfo(sampler);

        DescriptorSetBuilder()
            .BindImage(10, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &albedo)
            .BindImage(11, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &normals)
            .BindImage(12, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &metalicRougness)
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, transmittanceClusteredPipelineLayout, 0, 3, discriptorSets, 0, nullptr);

    vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(meshMaterial.indexCount), 1, static_cast<uint32_t>(meshMaterial.startIndex), 0, 0);
}

void MBOITStaticMeshPBRTexturePipeline::CreateAbsorbancePipeline(const GraphicsAPI& graphicsAPI, const SwapChain& swapChain, VkRenderPass absorbanceRenderPass)
{
    ...
    PipelineLayoutBuilder(logicalDevice)
        ...
        .AddPushConstant(sizeof(MBOITPushConstants), VK_SHADER_STAGE_FRAGMENT_BIT)
    ...
    pipelineBuilder
        ...
        .SetFragmentShader("shaders/Transparency/MBOIT/AbsorbancePassTexture.frag.spv", "main")
...
void MBOITStaticMeshPBRTexturePipeline::CreateTrasnmitanceClusteredPipeline(const GraphicsAPI& graphicsAPI, const SwapChain& swapChain, VkRenderPass transmittanceRenderPass, VkDescriptorSetLayout perTransmittancePassDescriptorSetLayout)
{
    const LogicalDevice& logicalDevice = graphicsAPI.GetLogicalDevice();
    VkDevice device = logicalDevice.GetVkDevice();
    ...
    perMaterialLayoutBuilder
        .AddBindPoint(10, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
        .AddBindPoint(11, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
        .AddBindPoint(12, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
    ...
    if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &transmittanceClusteredPipelineLayout) != VK_SUCCESS)
    {
    ...
    pipelineBuilder
        .SetVertexShader("shaders/Transparency/transparency.vert.spv", "main")
        .SetFragmentShader("shaders/Transparency/MBOIT/TransmittancePassTexture.frag.spv", "main")
        .SetVertexInputState(bindingDescription, attributeDescriptions)
        .SetInputAssemblyState(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, VK_FALSE)
        .AddViewport(viewport)
        .AddScissor(scissor)
        ...
        .SetPipelineLayout(transmittanceClusteredPipelineLayout)
