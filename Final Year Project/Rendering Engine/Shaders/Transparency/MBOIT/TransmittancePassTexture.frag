...
layout(set = 2, binding = 10) uniform sampler2D albedo;
layout(set = 2, binding = 11) uniform sampler2D normals;
layout(set = 2, binding = 12) uniform sampler2D metalicRoughness;
...
{
    vec3 N = normalize(NormalSampleToWorldSpace(texture(normals, inTexCoord).rgb, inNormal, inTangent, inBiTangent));
    ...
    for(int index = 0; index < tile_light_num; index++)
    {
        ...
        float NdotL = max(abs(dot(N, L)), 0.000001f);     
...
void main()
{
    ...
    material.metalness = metalicRoughness.r;
    material.roughness = metalicRoughness.g;

    vec3 finalLight = ShadeMaterial(material);
    
    outColor = vec4(finalLight * albedo.a * transmittance_at_depth, albedo.a * transmittance_at_depth);
}